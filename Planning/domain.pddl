...
(:action check_in
 ...
 :precondition
    ...
    (and 
        (has_reservation ?p ?r ?d) (is_current_day ?d) (is_vacant ?r) (is_clean ?r)
    )
...
(:action vacate
 :parameters (?r ?p ?d)
 :precondition 
    (and (has_room_key ?p ?r) (not (has_booked_room ?r ?d)) (is_current_day ?d))
 :effect 
    (and (not (has_room_key ?p ?r)) (is_vacant ?r))
)
...
(:action clean_room
 :parameters (?r)
 :precondition 
    (and (is_vacant ?r) (not (is_clean ?r)))
:effect 
    (is_clean ?r)
)
...
(:action next_day
 ...
 :precondition
	; TODO: fill in the necessary preconditions
    (and (is_current_day ?d1) (is_next_day ?d1 ?d2))
 :effect
	; TODO: fill in the resulting effects
    (and (is_current_day ?d2) (not (is_current_day ?d1)))
)
)